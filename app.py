...
from models import db, User, Exam, ExamQuestion, ExamSession, ExamResponse, Warning, LoginActivity, PasswordOTP, StudentQuery
...
def send_otp_email(receiver_email, otp):
    sender_email = "mukundmacharla2105@gmail.com"   # I will replace this
    sender_password = "nsgdwwnfhfnvhtmj"   # I will replace this
...
def forgot_password_request():
    ...
    PasswordOTP.query.filter_by(user_id=user.id, used=False).update({'used': True})
...
def admin_delete_exam_api(exam_id: int):
    ...
    e = Exam.query.get_or_404(exam_id)
    try:
        sessions = ExamSession.query.filter_by(exam_id=e.id).all()
        session_ids = [s.id for s in sessions]
        if session_ids:
            ExamResponse.query.filter(ExamResponse.session_id.in_(session_ids)).delete(synchronize_session=False)
...
@app.route('/admin/api/student_queries', methods=['GET'])
def admin_list_student_queries_api():
    if 'role' not in session or session.get('role') != 'admin':
        return jsonify({'success': False, 'message': 'Unauthorized'}), 403

    items = StudentQuery.query.order_by(StudentQuery.created_at.desc()).all()
    out = []
    for it in items:
        out.append({
            'id': it.id,
            'name': it.name,
            'email': it.email,
            'message_preview': (it.message[:80] + '...') if it.message and len(it.message) > 80 else (it.message or ''),
            'is_read': bool(it.is_read),
            'created_at': it.created_at.strftime('%Y-%m-%d %H:%M') if it.created_at else None,
        })
    return jsonify({'success': True, 'queries': out})


@app.route('/admin/api/student_queries/<int:query_id>', methods=['GET'])
def admin_get_student_query_api(query_id: int):
    if 'role' not in session or session.get('role') != 'admin':
        return jsonify({'success': False, 'message': 'Unauthorized'}), 403

    it = StudentQuery.query.get_or_404(query_id)
    if not it.is_read:
        it.is_read = True
        db.session.commit()
